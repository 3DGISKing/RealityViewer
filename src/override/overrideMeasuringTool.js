...
import {CameraMode, Measure, MeasuringTool, Utils} from "Potree"
...
// sphere
let nextCandidatePointSphere = new THREE.Mesh(sphereGeometry, createSphereMaterial());

const nextCandidatePointName = "next candidate point";

nextCandidatePointSphere.name = nextCandidatePointName;
...
function overrideMeasuringTool () {
    MeasuringTool.prototype.startInsertion = function(args = {}) {
        ...
        let insertionCallback = (e) => {
            if (e.button === THREE.MOUSE.LEFT) {
                // comment original potree code
                //measure.addMarker(measure.points[measure.points.length - 1].position.clone());

                // start reality
                if(measure.currentPointIndex !== -1){
                    return;
                }
        ...
        cancel.callback = e => {
            ...
            nextCandidatePointSphere.visible = false;
            // end reality
        };

        // start reality
        let onMouseMove = (e) => {
            const nextCandidatePoint = this.scene.getObjectByName(nextCandidatePointName);
            ...
            if(I) {
                nextCandidatePoint.position.copy(I.location);
                nextCandidatePoint.visible = true;
            }
            else {
                nextCandidatePoint.visible = false;
            }
        };
        // end reality
        ...
        // start reality
        if(!this.scene.getObjectByName(nextCandidatePointName)) {
            this.scene.add(nextCandidatePointSphere);
            console.log("added next candidate point");
        }
        // end reality
    ...
    MeasuringTool.prototype.update = function () {
        ...
        // start reality
        let distance = camera.position.distanceTo(nextCandidatePointSphere.getWorldPosition(new THREE.Vector3()));
        let pr = Utils.projectedRadius(1, camera, distance, clientWidth, clientHeight);
        let scale = (15 / pr);
        nextCandidatePointSphere.scale.set(scale, scale, scale);
        // end reality
